...
import { Alert, Badge, Button, Card, EmptyState, TriangleAlertIcon, UsersIcon } from '@thiagoschoeffel/ts-components'
...
<template>
  ...
  <div v-else class="space-y-4">
    ...
    <div class="grid gap-4 lg:grid-cols-2">
      <Card><template #header><h2 class="text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">Contato</h2></template><dl class="space-y-3"><div><dt class="text-xs font-medium uppercase tracking-wide text-slate-400">Nome</dt><dd class="mt-1 font-medium text-slate-800">{{ customer.name }}</dd></div><div><dt class="text-xs font-medium uppercase tracking-wide text-slate-400">Telefone</dt><dd class="mt-1 font-medium text-slate-800">{{ customer.phone }}</dd></div></dl></Card>
      <Card><template #header><h2 class="text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">Preferências operacionais</h2><p class="mt-1 text-sm text-slate-500">Defaults atuais, não fatos históricos.</p></template><dl class="space-y-3"><div><dt class="text-xs font-medium uppercase tracking-wide text-slate-400">Entregador</dt><dd class="mt-1 text-slate-700">{{ customer.preferredDeliveryPerson || 'Sem preferência' }}</dd></div><div><dt class="text-xs font-medium uppercase tracking-wide text-slate-400">Pagamento</dt><dd class="mt-1 text-slate-700">{{ [customer.preferredPaymentCondition, customer.preferredPaymentMethod].filter(Boolean).join(' · ') || 'Sem preferência' }}</dd></div></dl></Card>
    </div>
    <Card><template #header><h2 class="text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">Endereços</h2><p class="mt-1 text-sm text-slate-500">Endereços atuais usados como origem para novos pedidos.</p></template><Alert variants="neutral" size="small" description="Pedidos antigos preservam o endereço usado no momento da compra." /><div v-if="customer.addresses.length" class="mt-4 grid gap-3 md:grid-cols-2"><Card v-for="(address, index) in customer.addresses" :key="address.id"><p class="font-medium text-slate-800">{{ address.label || `Endereço ${index + 1}` }}</p><p class="mt-1 text-sm text-slate-500">{{ formatFullAddress(address) }}</p></Card></div><p v-else class="mt-4 text-sm text-slate-400">Nenhum endereço cadastrado.</p></Card>
    <div class="grid gap-4 lg:grid-cols-2">
      <Card><template #header><h2 class="text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">Preferências</h2><p class="mt-1 text-sm text-slate-500">Sugestões recorrentes que podem ser ajustadas em cada pedido.</p></template><div v-if="customer.preferences.length" class="flex flex-wrap gap-2"><Badge v-for="preference in customer.preferences" :key="preference.id" variant="neutral" size="medium">{{ preference.description }}</Badge></div><p v-else class="text-sm text-slate-400">Nenhuma preferência cadastrada.</p></Card>
      <Card><template #header><h2 class="text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">Restrições alimentares</h2><p class="mt-1 text-sm text-slate-500">Exigem verificação explícita ao montar o pedido.</p></template><Alert v-if="customer.dietaryRestrictions.length" variants="warning" size="small"><template #icon><TriangleAlertIcon /></template><ul class="space-y-1"><li v-for="restriction in customer.dietaryRestrictions" :key="restriction" class="font-medium">Restrição: {{ restriction }}</li></ul></Alert><p v-else class="text-sm text-slate-400">Nenhuma restrição alimentar cadastrada.</p></Card>
    </div>
    <Card><template #header><h2 class="text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">Observações</h2></template><p v-if="customer.notes" class="whitespace-pre-line text-slate-700">{{ customer.notes }}</p><p v-else class="text-slate-400">Nenhuma observação livre.</p></Card>
  </div>
</template>
