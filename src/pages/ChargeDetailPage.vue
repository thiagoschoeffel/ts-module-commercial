  ...
  <div v-else class="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
    <div class="space-y-4">
      ...
      <Card>
        ...
        <div v-if="allocations.length" class="divide-y divide-slate-100"><div v-for="allocation in allocations" :key="allocation.id" class="flex flex-col gap-2 py-3 first:pt-0 last:pb-0 sm:flex-row sm:items-center sm:justify-between"><div><p class="font-medium text-slate-800">{{ allocation.paymentId }} · {{ methodLabel(allocation.payment!.method) }}</p><p class="mt-1 text-xs text-slate-500">Recebido em {{ date(allocation.payment!.receivedAt) }} · registrado por {{ allocation.payment!.responsibleSnapshot }} em {{ dateTime(allocation.payment!.createdAt) }}</p></div><strong class="text-emerald-700">{{ currency(allocation.amount) }}</strong></div></div>
