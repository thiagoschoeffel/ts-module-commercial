...
interface ApiCommerce {
  ...
  charges: Array<{ id: string, customerId: string, customerNameSnapshot: string, orderId: string, amount: number, dueOn: string, createdAt: string, cancelledAt?: string, status: 'Pending'|'Cancelled' }>
...
export async function reloadCommerce() {
  ...
  snapshot = {
    ...
    charges: data.charges.map(c => ({ id: c.id, customerId: c.customerId, customerNameSnapshot: c.customerNameSnapshot, orderId: c.orderId, description: `Pedido ${c.orderId.slice(0, 8).toUpperCase()}`, amount: c.amount, dueDate: c.dueOn, createdAt: c.createdAt, canceledAt: c.cancelledAt })),
...
export async function registerPaymentApi(input:RegisterPaymentInput) {
  const payment = await json<{id:string}>('/api/payments', { method:'POST', headers:{...headers,'Idempotency-Key':input.idempotencyKey}, body:JSON.stringify({ customerId:input.customerId, amount:input.amount, receivedOn:input.receivedAt, method:({pix:'Pix',cash:'Cash','debit-card':'DebitCard','credit-card':'CreditCard','bank-transfer':'BankTransfer'} as const)[input.method], reference:input.reference, allocations:input.allocations }) })
  try {
    await reloadCommerce()
    return { payment, synchronized: true as const }
  }
  catch {
    return { payment, synchronized: false as const }
  }
}
